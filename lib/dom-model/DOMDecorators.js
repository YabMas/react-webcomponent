...
function makeShadowNode(target, selector) {

    // const shadow = target.cloneNode(true);

    const uuid = crypto.randomUUID();

    // shadow.classList.add("shadow-node");
    // shadow.dataset.shadow = uuid;

    target.dataset.shadow = uuid;

    const dataNode = new ShadowNode();
    dataNode.node = target;
    dataNode.shadow = uuid
    // dataNode.shadow = shadow;
...
function attachContentListObserver(target, key, element, valueFn, observeOptions) {
    if (element && !element._isObserved) {
        const observer = new MutationObserver(() => {
            console.log("Updated content list: ")
            console.log(key)
...
function attachReactChildrenObserver(target, key, element, valueFn, observeOptions) {
    if (element && !element._isObserved) {
        const observer = new MutationObserver((mutations) => {
            console.log("Updated react child: ")
            console.log(key)
            console.log(mutations)
            const val = valueFn && valueFn(element)
            if (val) {
                element.dispatchEvent(new CustomEvent('_updateModel', {
                    detail: [{
                        propertyName: key,
...
let reactChildren = makeDecorator(function (selector) {
    return function (target, key, descriptor) {
        descriptor.writable = true;
        if (target.addProperty) {
            target.addProperty(key, (element) => {
                if (element && (element instanceof HTMLElement)) {
                    ...
                    let valueFn = function (domElement) {
                        console.log("In valueFn")

                        let newChildren = [...queryChildren(element, selector.concat(":not([data-shadow])"), true)]
                        ...
                        if (newChildren.length > 0) {
                            ...
                            newChildren.forEach(child => {
                                child.hidden = true;
                                const node = makeShadowNode(child, selector);
                                nodes.set(node.shadow, <PlaceholderNode item={node}/>)});
                        }
                    ...
                    // let valueFn = () => {
                    //     if (!nodes) {
                    //         let children = [...queryChildren(element, selector, true)];
                    //
                    //         // ".dropdown-item:not([data-shadow])"
                    //
                    //         if (children) {
                    //             nodes = children.map(child => <PlaceholderNode item={makeShadowNode(child, selector)}/>);
                    //             // nodes = children.map(child => <EmbedNode item={makeDOMNode(child, selector)}/>);
                    //         }
                    //     }
                    //     return nodes;
                    // }

                    attachReactChildrenObserver(target, key, element, valueFn, {
