    ...
    if (element && !element._isObserved) {
        const observer = new MutationObserver((mutations) => {
            element.dispatchEvent(new CustomEvent('_updateModel', {
                detail: [{
                    propertyName: key,
                    value: valueFn && valueFn(mutations)
                }]
            }))
        });
        element._isObserved = true;
        ...
        if (target.addProperty) {
            target.addProperty(key, (element) => {
                if (element && (element instanceof HTMLElement)) {
                    ...
                    let nodes = new Map();
                    let valueFn = function (mutations) {

                        console.log("Mutations:")
                        console.log(mutations)

                        if (mutations) {
                            let actualMutation = false;
                            mutations.forEach(mutation => {
                                if (mutation.addedNodes.length === 1 && mutation.removedNodes.length === 1) {
                                    console.log("Only moved nodes");
                                    actualMutation = true;
                                } else {
                                    console.log("Actual mutation")
                                    if (mutation.addedNodes.length === 0 && mutation.removedNodes.length === 1) {
                                        console.log("Node deleted")
                                        const removed = mutation.removedNodes[0].dataset.shadow;
